...
object Examples {
  ...
  val prod1 = StrSymb to StrSymb using {
    case ExpStr(str) :: HNil => OrderedStream.unit(Scored(ExpStr(s"($str)"), 1.0))
  }
  val prod2 = (IntSymb, StrSymb) to StrSymb using {
    case ExpInt(i) :: ExpStr(str) :: HNil => OrderedStream.unit(Scored(ExpStr(s"${i * i}:$str"), 1.0))
  }
  val prod3 = (IntSymb, Terminal("+"), IntSymb) to IntSymb using {
    case ExpInt(i) :: _ :: ExpInt(i2) :: HNil => OrderedStream.unit(Scored(ExpInt(i + i2), 1.0))
  }
